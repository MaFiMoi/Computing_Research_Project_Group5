...
export const Hero = () => {
  // MỚI: Tạo các state để quản lý dữ liệu nhập vào, kết quả AI, và trạng thái loading
  const [prompt, setPrompt] = useState('');
  const [response, setResponse] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  // MỚI: Hàm xử lý khi người dùng gửi form
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); // Ngăn trang tải lại
    setIsLoading(true);
    setResponse(''); // Xóa kết quả cũ

    // Tạo một câu prompt đầy đủ hơn cho AI
    const fullPrompt = `Hãy phân tích mục sau đây để xem nó có dấu hiệu lừa đảo hay không: "${prompt}". Vui lòng cung cấp phân tích ngắn gọn, đi thẳng vào vấn đề.`;

    try {
      // Gọi đến API /api/chat mà chúng ta đã tạo
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ prompt: fullPrompt }), // Gửi câu prompt đầy đủ
      });

      const data = await res.json();

      if (res.ok) {
        setResponse(data.response); // Cập nhật state với kết quả từ AI
      } else {
        setResponse(`Đã xảy ra lỗi: ${data.error}`);
      }
    } catch (error) {
      setResponse('Không thể kết nối đến máy chủ. Vui lòng thử lại.');
    } finally {
        ...
        <div className="flex items-center w-full lg:w-1/2">
          <div className="max-w-2xl mb-8">
            ...
            <form
              ...
              <button
                type="submit"
                className="px-8 py-3 text-lg font-medium text-center text-white bg-indigo-600 rounded-md whitespace-nowrap hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50"
                // MỚI: Vô hiệu hóa nút khi đang tải
                disabled={isLoading}>
                {/* MỚI: Thay đổi text khi đang tải */}
                {isLoading ? 'Đang phân tích...' : 'Tra cứu'}
              </button>
            </form>

            {/* MỚI: Khu vực hiển thị kết quả (popup) */}
            {response && !isLoading && (
              <div className="mt-6 p-4 bg-gray-100 border border-gray-200 rounded-md dark:bg-gray-800 dark:border-gray-700">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                  Kết quả phân tích:
                </h3>
                {/* Dùng 'whitespace-pre-wrap' để giữ nguyên các dòng mới từ AI */}
                <p className="mt-2 text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
                  {
                   response.replace(/^###\s?/gm, '')
                  }
                </p>
              </div>
            )}
            {/* --- PHẦN ĐƯỢC THAY THẾ KẾT THÚC --- */}
          </div>
        </div>
...
// (Phần code của MicrosoftLogo giữ nguyên)
